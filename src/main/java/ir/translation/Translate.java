...
//            throw new Error("Translate: Unknown primitive type, please report");
    ...
    private static class TranslationVisitor implements ast.Visitor<ir.translation.Result> {
        ...
        public Result visit(InsIf stm) {
            ...
            Label elseIfConditions = Label.fresh();
            ...
            List<Command> elseBranch = hasElse ?
                    ...
                    new LinkedList<>();
            Command conditional = new ir.com.CJump(condition.getExp(), thenLabel, elseIfConditions);
            List<Command> code = condition.getCode();
            code.add(conditional);
            code.add(elseIfConditions);
            Map<Label, Instruction> elseIfBodies = new HashMap<>();
            for (Pair<Expression, Instruction> currentElseIf : stm.getElseif()) {
                Result currentConditionCode = currentElseIf.getFst().accept(this);
                Label nextElseIfConditionLabel = Label.fresh();
                Label currentElseIfBody = Label.fresh();
                elseIfBodies.put(currentElseIfBody, currentElseIf.getSnd());
                code.addAll(currentConditionCode.getCode());
                code.add(
                        new CJump(
                                currentConditionCode.getExp(),
                                currentElseIfBody,
                                nextElseIfConditionLabel
                        )
                );
                code.add(nextElseIfConditionLabel);
            }
            if (hasElse) code.add(new Jump(elseLabel));
            else code.add(new Jump(exitLabel));
            code.add(thenLabel);
            code.addAll(thenBranch);
            code.add(new Jump(exitLabel));
            for (Map.Entry<Label, Instruction> current_body : elseIfBodies.entrySet()) {
                code.add(current_body.getKey());
                code.addAll(current_body.getValue().accept(this).getCode());
                code.add(new Jump(exitLabel));
            }
            if (hasElse) {
                code.add(elseLabel);
